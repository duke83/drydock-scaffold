...
function writeMocks(ip, port, destination, transactions) {
  ...
  var transactionsFiltred = transactions.filter(function(trans){
    return (
            (trans.method==='POST' && trans.pathname==='/typeAheadService') ||
            (trans.method==='POST' && trans.pathname==='/facetService') ||
            (trans.method==='POST' && trans.pathname==='/tableService') ||
            (trans.method==='POST' && trans.pathname==='/summaryLevelGeoService') ||
            (trans.method==='POST' && trans.pathname==='/summaryLevelService') ||
            (trans.method==='POST' && trans.pathname==='/renderService') ||
            (trans.method==='POST' && trans.pathname==='/geoNameService')   
    )
  })
  ...
  var routesNodes = Object.keys(routes).map(function (routeKey) {
    ...
    var handlersNode = (0, _handlers2.default)(route.responses.map(function (response) {
      ...
      return {
        ast: (0, _handler2.default)(response.uniqueName, 
              pathToFixture, 
              route.isJson, 
              response.statusCode,
              JSON.stringify(response.requestQueryString),
              JSON.stringify(response.requestPostData)
              ),
